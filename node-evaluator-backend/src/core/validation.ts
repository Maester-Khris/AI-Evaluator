...
export const isValidRating = (v: any) => typeof v === 'number' && v >= 1 && v <= 5;
 ...
 */
export function validate(source: 'body' | 'params' | 'query', schema: Record<string, Validator>): RequestHandler {
	return (req: Request, _res: Response, next: NextFunction) => {
		const errors: string[] = [];
		const target: any = (source === 'body') ? req.body : (source === 'params') ? req.params : req.query;
